...
if(isset($_POST['search']))
{
    ...
    try {
        ...
        $stmt = GetStatement($con, $query, $namearray);
        $count = $stmt->rowCount();
    ...
    try {
        ...
        $stmt = GetStatement($con, $query2, $dra_id_suche);
        $name = $stmt->fetch();
       ...
       // $stmt = $con->GetStatement($query, $dra_id_suche);
        $stmt = GetStatement($con, $query, $dra_id_suche);

        echo '<br><h4>Alle Ergebnisse für '.$name[0].':</h4>';
       // $con->PrintTable($stmt);
        PrintTable($con,$query,$dra_id_suche);
